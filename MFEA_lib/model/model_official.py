...
class model(AbstractModel.model):
    ...
    def fit(self, nb_inds_each_task: int, nb_generations :int ,  nb_inds_min:int, num_eval_task_per_epoch: int, evaluate_initial_skillFactor = False,LSA = False,
            *args, **kwargs): 
        ...
        while np.sum(eval_k) < MAX_EVALS_PER_TASK*num_tasks and stop == False:
            ...
            if np.sum(eval_k) >= epoch * nb_inds_each_task * num_tasks:
                ...
                # self.render_process(epoch/nb_generations, ['Pop_size', 'Cost'], [[len(population)], self.history_cost[-1]], use_sys= True)
                epoch +=1
            if np.sum(eval_k) >= num_record * num_eval_task_per_epoch * num_tasks:
                # save history
                self.history_cost.append([ind.fcost for ind in population.get_solves()])
                self.render_process(epoch/nb_generations, ['Pop_size', 'Cost'], [[len(population)], self.history_cost[-1]], use_sys= True)
                # epoch +=1
